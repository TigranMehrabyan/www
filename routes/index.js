...
router.get('/', function(req, res) {
	...
	req.db.Stores.find({$not: {status: 2}}).sort({date: -1}).limit(20).exec(function(err, stores) {
...
router.get('/shops', function(req, res) {
	...
	var page = req.query.p || 1; page *= 1;

	var sortArray = ["name", "date", "cashback", "purchases"];

	var sort = utils.sorting(sortArray, 2, req.query);
	...
	req.db.Categories.find({}).sort({name3: 1}).exec(function(err, categories) {
		...
		if (!err && categories.length) {
			...
			if (createCategories.data) {
				...
				req.data.catgoryName =createCategories.data.name;
			...
			{
					req.data.catgoryName = req.data.langs.titles.index_shops;
			}
		...
		req.db.Stores.find(findQuery).sort(sort).skip((perPage * page) - perPage).limit(perPage).exec(function(err, stores) {
			...
			req.db.Stores.count(findQuery).exec(function(err, count) {
				...
				req.db.Products.find({$not: {available: false}}).sort({ modified_time: -1}).limit(4).exec(function(err, right_products) {

					if (right_products && right_products.length) {
                                            
                                            console.log(right_products.length);
...
try {
	...
	if (!storeId) {
	 ...
	 req.db.Stores.findOne({_id: storeId, $not: {status: 2}}, async function(err, store) {
		...
		let regEx = new RegExp(store.offer_id);
		//console.log(regEx);

              //  let results = await utils.parseShopProdutsFeed(configs, req.db, store.feed_url, store.offer_id, store._id); 


		req.db.Promocodes.find({advcampaign_id: {$regex: regEx}}).sort({date_start: -1}).limit(10).exec(function(err, relPromocodes) {

			req.data.relPromocodes = relPromocodes;
...
router.get('/shop/products', function(req, res) {
try {
	...
	req.db.Stores.findOne({_id: storeId, $not: {status: 2}}, async function(err, store) {
		...
		let regEx = Number(store.offer_id);
		console.log(regEx);

              //  let results = await utils.parseShopProdutsFeed(configs, req.db, store.feed_url, store.offer_id, store._id); 


		req.db.Products.find({store_offer_id: regEx}).sort(sort).skip((perPage * page) - perPage).limit(perPage).exec(function(err, products) {
                        req.db.Products.count({store_offer_id: regEx}).exec(function(err, count) {
				...
				if (products && products.length) {
				 ...
				 res.render('products_to_store', req.data);
...
router.get('/blackfriday', function(req, res) {

	req.data.title = req.data.langs.how_works_cashback;

	req.data.navLinks = [{url: req.url, title: req.data.title}];

	res.render('blackfriday', req.data);

});


router.get('/ramadan', function(req, res) {

	req.data.title = req.data.langs.how_works_cashback;

	req.data.navLinks = [{url: req.url, title: req.data.title}];

	res.render('ramadan', req.data);

});


router.get('/cybermonday', function(req, res) {

	req.data.title = req.data.langs.how_works_cashback;

	req.data.navLinks = [{url: req.url, title: req.data.title}];

	res.render('cybermonday', req.data);
...
router.get('/hot_deals', function(req, res) {
	...
	req.db.Categories.find({}).sort({name3: 1}).exec(function(err, categories) {
		...
		var findQuery = {$not: {available: false}};
...
router.get('/promocodes', async function(req, res) {
	...
	try {
		...
		req.data.title = "Промокоди";

		req.data.navLinks = [{url: '/promocodes', title: req.data.title}];

		var perPage = 6, page = req.query.p || 1; page *= 1;

		var sortArray = ["name", "date_start", "date_end", "discount"];
		...
		req.db.Categories.find({}).sort({name3: 1}).exec(function(err, categories) {
			...
			if (!err && categories.length) {
				...
				req.data.categoryPath = 'promocodes';
			...
			if (createCategories.data) {
				...
				if (createCategories.data.childs && createCategories.data.childs.length) {
					...
					findQuery = {$or: findCategoriesIds};

				} else if (createCategories.data.parentId) {

					findQuery = {$or: [{categories: category}, {categories: createCategories.data.parentId}]};

				}

			}

				req.db.Promocodes.find({ specie_id: '1' },findQuery).sort(sort).skip((perPage * page) - perPage).limit(perPage).exec(function(err, promocodes) {

				req.db.Promocodes.count({ specie_id: '1' },findQuery).exec(function(err, count) {
					...
					if (promocodes && promocodes.length) {
						...
						for (var i = 0; i < promocodes.length; i++) {
							...
							/*if (promocodes[i].endDate && promocodes[i].endDate < nowTime) {

								req.db.Products.remove({_id: promocodes[i]._id});

								promocodes.splice(i, 1); i--;
						...
						req.data.promocodes = promocodes;
					...
					req.db.Products.find({$not: {status: 2}}).sort({purchases: -1, date: -1}).limit(4).exec(function(err, right_products) {
						...
						res.render('promocodes', req.data);
		...
		let results = await utils.parsePromocodesFeed(configs, req.db);

		if (results && results.error) console.log(results.error);
...
router.get('/actions', async function(req, res) {
	...
	try {
		...
		req.data.title = "Акції";

		req.data.navLinks = [{url: '/actions', title: req.data.title}];
		...
		req.db.Categories.find({}).sort({name3: 1}).exec(function(err, categories) {
			...
			req.db.Promocodes.find({ specie_id: '2' },findQuery).sort(sort).skip((perPage * page) - perPage).limit(perPage).exec(function(err, promocodes) {

				req.db.Promocodes.count({ specie_id: '2' },findQuery).exec(function(err, count) {
...
router.get('/promocode', function(req, res) {
	...
	var promocodeId = req.query.id || false;
	...
	req.db.Promocodes.findOne({_id: promocodeId}, function(err, promocode) {
		...
		req.data.promocode = promocode;

		req.data.title = req.data.langs.promocode + ' ' + promocode.name;

		req.data.metaDesc = req.data.title + '. ' + configs.metaDescDefault;

		req.data.metaKeys = promocode.name + ', ' + configs.metaKeysDefault;

		req.data.pDescription = promocode.description;



		switch (promocode.types.type_id[0]) {
  case "1":
   req.data.pTypes = req.data.langs.promo_type.first;
    break;
  case "2":
    req.data.pTypes = req.data.langs.promo_type.second;
    break;
  case "3":
    req.data.pTypes = req.data.langs.promo_type.theee;
    break;
  default:
    req.data.pTypes = "ttt";
}
		...
		//console.log(req.data.pTypes);


              


		var st = promocode.advcampaign_id;


		req.db.Stores.findOne({offer_id: parseInt(st)}, function(err, nStore) {

		req.data.storelink =false;

		if (nStore) {



				req.data.storelink = nStore._id;

				console.log(req.data.storelink);

			}
		if(promocode.specie_id == '2')
		{
			var url = '/actions';
			var url_name = req.data.langs.hot_actions;
		}
		else
		{
			var url = '/promocodes';
			var url_name = req.data.langs.hot_deals;
		}


		req.data.navLinks = [{url: url, title: url_name}, {url: req.url, title: promocode.name}];
		...
		findQuery = {$not: {_id: promocode._id}, $and: [{advcampaign_id: promocode.advcampaign_id}, { specie_id: '1' }]};

		req.db.Promocodes.find(findQuery).sort({date_start: -1}).limit(35).exec(function(err, relPromocodes) {

			if (relPromocodes && relPromocodes.length) {

				for (var i = 0; i < relPromocodes.length; i++) {
				...
				req.data.relPromocodes = relPromocodes;
					...
					var findCategoriesIds = [];
					if(promocode.categories.category_id.length && promocode.categories.category_id)
					{
					for (var c = 0; c < promocode.categories.category_id.length; c++) {

						findCategoriesIds.push(Number(promocode.categories.category_id[c]));
				...
				findQuery = { id: { $in: findCategoriesIds } };
                        
                      
                   
                        req.db.Promocodcategory.find(findQuery).sort({id: -1}).exec(function(err, promocodcategory) {
                        
                      let catnames= promocodcategory[0].name;
                        
                        for (var u = 1; u < promocodcategory.length; u++) {

					catnames += ", "+promocodcategory[u].name;

					}
                          
                          req.data.catnames = catnames;
            
                       
                        
                        

			res.render('promocode', req.data);
...
router.get('/product', function(req, res) {
	...
	var productId = req.query.id || false;
	...
	req.db.Products.findOne({_id: productId}, function(err, product) {
		...
		req.data.title = req.data.langs.product + ' ' + product.name;

		req.data.metaDesc = req.data.title + '. ' + configs.metaDescDefault;

		req.data.metaKeys = product.name + ', ' + configs.metaKeysDefault;

		req.data.pDescription = product.desc_1;
...
*/
		...
		req.data.navLinks = [{url: '/hot_deals', title: req.data.title}, {url: req.url, title: product.name}];
