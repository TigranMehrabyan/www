...
router.get('/sendVerificationAgain', function(req, res) {

  if (!req.data.user) return res.redirect("back");

  if (req.data.user.verified_email == 1) return res.redirect("back");

  var noverifiedCode = utils.getRandomInt(10000, 99999) + Date.now();

  req.db.Users.update({_id: req.data.user._id}, {$set: {noverified: noverifiedCode}}, function(err) {

    var mailOptions = {

      to: req.data.user.email,
      html: 'activation',
      lang: req.data.lang
