...
router.get('/shops', function(req, res) {
	...
	var perPage = 600, page = req.query.p || 1; page *= 1;

	var sortArray = ["name", "cashback", "purchases", "profit", "status", "date"];
...
router.get('/updateproductsfeed', function(req, res, next) {

	var db = req.db, storeId = req.query.id;
        var configs = req.data.configs;
	if (!storeId) return res.redirect('back');
        

	req.db.Stores.findOne({_id: storeId}, async function (err, store) {

		if (!store) return res.redirect('back');
...
router.post('/api/editshop', uploadPicture.array("picture"), function(req, res) {
	...
	if (!data || !data.store_id || !data.name || !data.affilnet || !data.categories || !data.countries || !data.url || !data.cashback || !data.cashback_k || !data.average_time || !data.max_time || !data.desc_1 || !data.desc_2 || !data.desc_3) return res.json({error: "Не заполнены все обязательные поля!"});
...
router.post('/api/editcategory', function(req, res) {
	...
	if (!data || !data.category_id || !data.category_name1 || !data.category_name2 || !data.category_name3 || (!data.parent_category && !data.category_icon) || !data.category_desc_1 || !data.category_desc_2 || !data.category_desc_3) return res.json({error: "Не заполнены все обязательные поля!"});

	var updateData = {

		name1: data.category_name1,
		name2: data.category_name2,
                name3: data.category_name3,
		icon: data.category_icon,
		desc_1: data.category_desc_1,
		desc_2: data.category_desc_2,
                desc_3: data.category_desc_3
...
router.post('/api/settings', function(req, res) {
	...
	if (data.sitename2 && data.sitename2 !== configs.siteName2) updateData.siteName2 = data.sitename2;
	if (data.sitename3 && data.sitename3 !== configs.siteName3) updateData.siteName3 = data.sitename3;

	if (data.metadescdefault && data.metadescdefault !== configs.metaDescDefault) updateData.metaDescDefault = data.metadescdefault;
	if (data.metadescdefault2 && data.metadescdefault2 !== configs.metaDescDefault2) updateData.metaDescDefault2 = data.metadescdefault2;
	if (data.metadescdefault3 && data.metadescdefault3 !== configs.metaDescDefault3) updateData.metaDescDefault3 = data.metadescdefault3;

	if (data.metakeysdefault && data.metakeysdefault !== configs.metaKeysDefault) updateData.metaKeysDefault = data.metakeysdefault;
	if (data.metakeysdefault2 && data.metakeysdefault2 !== configs.metaKeysDefault2) updateData.metaKeysDefault2 = data.metakeysdefault2;
	if (data.metakeysdefault3 && data.metakeysdefault3 !== configs.metaKeysDefault3) updateData.metaKeysDefault3 = data.metakeysdefault3;
	...
	if (data.promocodesXML && data.promocodesXML !== configs.promocodesXML) updateData.promocodesXML = data.promocodesXML;
        if (data.promocodesXML2 && data.promocodesXML2 !== configs.promocodesXML2) updateData.promocodesXML2 = data.promocodesXML2;
        if (data.promocodesXML3 && data.promocodesXML3 !== configs.promocodesXML3) updateData.promocodesXML3 = data.promocodesXML3;
        if (data.promocodesXML4 && data.promocodesXML4 !== configs.promocodesXML4) updateData.promocodesXML4 = data.promocodesXML4;
        if (data.promocodesXML5 && data.promocodesXML5 !== configs.promocodesXML5) updateData.promocodesXML5 = data.promocodesXML5;
        if (data.promocodesXML6 && data.promocodesXML6 !== configs.promocodesXML6) updateData.promocodesXML6 = data.promocodesXML6;
        if (data.promocodesXML7 && data.promocodesXML7 !== configs.promocodesXML7) updateData.promocodesXML7 = data.promocodesXML7;
        if (data.promocodesXML6 && data.promocodesXML8 !== configs.promocodesXML8) updateData.promocodesXML8 = data.promocodesXML8;
        if (data.promocodesXML9 && data.promocodesXML9 !== configs.promocodesXML9) updateData.promocodesXML9 = data.promocodesXML9;
        if (data.promocodesXML10 && data.promocodesXML10 !== configs.promocodesXML10) updateData.promocodesXML10 = data.promocodesXML10;
...
router.get('/import_promocodes', async function(req, res) {

	try {

		/*let results = await utils.parsePromocodesFeed(req.data.configs, req.db);

		if (results.error) return console.log(results.error);

		res.json({success: "Промокоды успешно импортированы (" + results.count + ")!"});*/
